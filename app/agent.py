...
class FinancialAgent:
    ...
    @staticmethod
    def _money(v: float) -> float: return round(float(v) + 1e-9, 2)
    ...
    def _option_candidates(self, request: pd.Series, profile: pd.Series) -> list[dict[str, Any]]:
        accepted = self.engine.accepted_methods(profile); rows = self.data["payment_options"][self.data["payment_options"].request_id.eq(request.request_id)]
        candidates = []
        for _, opt in rows.iterrows():
            ...
            if payments and payments[-1].date > pd.Timestamp(request.desired_completion_date): continue
            sim = self.engine.simulate(request.user_id, request.request_date, payments)
            if sim.safe: candidates.append({"kind": method, "option": opt, "payments": payments, "sim": sim, "fee": float(opt.financing_fee), "total": float(opt.total_payable_amount)})
        return candidates
    ...
    def _partial_candidate(self, request: pd.Series, amount_today: float, date_full: pd.Timestamp | None) -> dict[str, Any] | None:
        if not bool(request.allows_partial_payment) or amount_today <= 1e-8 or amount_today >= float(request.requested_amount) - 1e-8 or date_full is None or date_full > pd.Timestamp(request.desired_completion_date): return None
    ...
    def _change_candidate(self, request: pd.Series, base: dict[str, Any]) -> dict[str, Any] | None:
        ...
        # Exhaustively try up to three independent flexible changes; this covers the challenge's multi-change cases.
        for n in range(1, min(3, len(changes)) + 1):
            for combo in combinations(changes, n):
                ...
                if sim.safe: return {**base, "sim": sim, "changes": list(combo), "change_count": n}
        return None

    def _rank(self, candidate: dict[str, Any], request: pd.Series) -> tuple:
        payments = candidate["payments"]; completes = bool(payments) and payments[-1].date <= pd.Timestamp(request.desired_completion_date)
        changes = len(candidate.get("changes", [])); start = payments[0].date if payments else pd.Timestamp.max; count = len(payments)
    ...
    def decide(self, request: pd.Series) -> dict[str, Any]:
        profile = self.engine.profile(request.user_id); requested = float(request.requested_amount)
        ...
        interpretation = self.ai.interpret(str(request.request_text)); risk = self.risk.score(profile)
        ...
        if not candidates and "full_payment" in self.engine.accepted_methods(profile):
            changed = self._change_candidate(request, full)
            if changed: candidates.append(changed)

        if candidates:
            best = min(candidates, key=lambda x: self._rank(x, request)); method = best["kind"]
            ...
            if method == "full_payment" and best.get("option") is None and earliest is not None and earliest > pd.Timestamp(request.request_date): status, method = "affordable_later", "wait"
            changes = best.get("changes", []); change_text = "none" if not changes else "|".join((f"stop:{c['event_id']}" if c["action"] == "stop" else f"reduce_to:{c['event_id']}:{self._money(c['new_amount']):.2f}") for c in changes)
            ...
            if changes: reason += " Flexible recurring spending is adjusted to preserve the safety buffer."
            return {"request_id": request.request_id, "amount_safe_to_pay": self._money(today_safe), "affordability_status": status, "recommended_payment_method": method, "payment_plan": plan, "earliest_date_for_full_payment": "" if earliest is None else f"{earliest:%Y-%m-%d}", "spending_changes_needed": change_text, "decision_explanation": reason, "_risk_score": risk, "_ai_intent": interpretation.get("intent", "other")}

        if earliest is not None and "full_payment" in self.engine.accepted_methods(profile):
            return {"request_id": request.request_id, "amount_safe_to_pay": self._money(today_safe), "affordability_status": "affordable_later", "recommended_payment_method": "wait", "payment_plan": f"{earliest:%Y-%m-%d}:{requested:.2f}", "earliest_date_for_full_payment": f"{earliest:%Y-%m-%d}", "spending_changes_needed": "none", "decision_explanation": f"Wait until {earliest:%Y-%m-%d}; paying earlier would breach the minimum balance constraint.", "_risk_score": risk, "_ai_intent": interpretation.get("intent", "other")}
        return {"request_id": request.request_id, "amount_safe_to_pay": self._money(today_safe), "affordability_status": "not_affordable", "recommended_payment_method": "not_recommended", "payment_plan": "none", "earliest_date_for_full_payment": "" if earliest is None else f"{earliest:%Y-%m-%d}", "spending_changes_needed": "none", "decision_explanation": "Do not proceed. No eligible plan completes the full request safely while maintaining the required minimum balance.", "_risk_score": risk, "_ai_intent": interpretation.get("intent", "other")}
